...
#                                 nvreport.py                                       *
#                                                                                   *
#  Input is one or more i-360 survey file(s) and optional question text file         *
#        may be in either .csv or .xlsx format                                      *
#                                                                                   *
#  Output is a text report of the survey and analysis thereof  jes    
#  input can be either:     
#       -a single survey file            < -infile survey-file-name.csv >           *
#       -a single SurveyResponse* file   < -infile i360 >                           *
#       -a direcotry of survey files     < -survey ~/Downloads  >                   *
#       -a special selection by name patterns (not used)
#                                                                                   *
# *********************************************************************************** 
...
def printhelp():
    ...
    print ("                    -pers C,M,MC\n")
    print ("    -qfile <filename> text file of question text to substitute for the i-360 text.\n")
...
#
#   row_cols = list of columns in this row
#
#   return 0 if all questions answered, 1 if only some answered
#-----------------------------------------------------------------
def doquestions (row_cols):
    ...
    for q in range(0,NumQuestions):
        rtext = (row_cols[FirstQCol+q])                    # fetch response to this question
        if (pd.isna(rtext)):
...
def main():
    ...
    #
    global infile, voterHistoryFiledatadir, rptfile, svyfile, svydir, qfile, sframe, printFileh, outfh
    ...
    try:
        ...
        exit(2)
    #
    path = pathlib.PurePath(sys.argv[0][0:-3])      # Stash program Name (minus .py) in upper case for PrintLine
    ProgName = path.name.upper()                    
    ...
    #  - uuid.zip
    #  - SurveyResponse*any_name.xlsx, SurveyResponse*anyname_.csv
    #  - some variants vote
    #
    UUID_PATTERN = re.compile(r'^[\da-f]{8}-([\da-f]{4}-){3}[\da-f]{12}$', re.IGNORECASE)
    #
    # Chekck to see if looking for single  -infile
    if (infile != "" and infile != "i360"):
        SurveyFiles.append(infile)                                     # infile overrides any selection of files
 
    #  If survey directory given - use it
    if  (svydir) != "":
        curdir = svydir
    else:
        curdir =  os.getcwd() 
    # if a Survey Subdirectory specified add it to path
        #dir = f"{dir}/{svydir}"                                     # if a Survey Subdirectory specified add it to path
    printLine(f"Survey Directory = {curdir}")
    if (os.path.isdir(curdir) == False):
        printLine (">>> Directory does not exist --- aborting!\n")
        exit(2)

    #[[[]]] Look for "SurveyRespone" key requested in selection
    #[[[]]] - if so scan directory for SurveyResponse, or imbedded uuid.zip in filename
    '''
    if (infile == "i360"):
        # read either cwd or surveydir 
        surveylist = os.listdir(curdir)
        
        # look for "SurveyResponse" in name and store in SurveyFiles
        for infile in surveylist:
            if "SurveyResponse" in infile:
                SurveyFiles.append(infile)

        # look for "embedded uuid".zip in name and store in SurveyFiles
        for infile in surveylist:
            infile3 = infile[:36]     # only use the first 36 char of UUID 
            if UUID_PATTERN.match(os.path.splitext(infile3)[0]):
                if infile.endswith(".zip"):
                    SurveyFiles.append(infile)       
        
        #allfiles =  listdir(curdir) if isfile(join(edir, f))]    # get list of files in survey directory
        '''
    sfile = ""
        #
        #  Build list of only survey file names
        ...
        #  - some variants vote
        #
    for sfile in surveylist:                                   # loop through file names
        temp = sfile                                          # get next file name
        temp = temp.lower()  
            '''
            if (temp[0] == '.'):                              # remove a hidden
                continue
            if temp.endswith(".xlsx"):
                continue
            temp = temp.replace(".xlsx","")                             # it's a .xlsx file, remove file extension
            flag = 0  
                                                          # assume good survey file
            #if temp.startswith("i-20p" ):
            #    flag = 1
            if ((temp[0] == "p") and (temp[2] == "-")):                 # if name starts with i-20p or pn- it's a survey
                flag = 1
            if(flag == 0):
        if "SurveyResponse" in temp:
            SurveyFiles.append(f"{dir}{sfile}")          # This file is selected add to survey file name list
    ...
    #  Check for a .zip file and process it
    #  - 
    #
    for svyfile in SurveyFiles:                                 # get next file name to process
        if (svyfile == SurveyFiles[0]):
    
    #  check to see if this is a zip'ped survey file
    #  call unzip routine to unzip and 
    #  rename and add unzipped file to the stack
            #ext = os.path.splitext(svyfile)[-1].lower()
            if svyfile.endswith(".zip"):
                with ZipFile(svyfile, 'r') as zip:
                    zip.extractall("zipps")
                    my_path = 'zipps/exports/'
                    files = os.listdir(my_path)
                        # now look in exports/ directory to see if ant .csv
                        # - if so read file into dataframe an look for Title column
                        # - if a Title exists then rename file to SurveyResponse-{title}
                        # - replace file in SurveyFiles with new file
                    for file in files:
                        if os.path.isfile(os.path.join(my_path, file)):
                            if file.endswith(".csv"):
                                sframe = pd.read_csv (os.path.join(my_path, file), low_memory=False, encoding='latin-1') 
                                SurveyTitle = sframe.iloc[0][0]
                                #file_newname = "SurveyResponse-" + file #SurveyTitle 
                                file_newname = file
                                #dir = os.getcwd()                                               # Get our current working directory
                                svyfile = (os.path.join(dir,my_path + file_newname))
                        
            #  check file name to see if .csv or .xlsx and make each
            #  - use either read_csv or read_excel as needed to allow full
            #
            printLine (f"Loading {svyfile} ... ")
            if svyfile.endswith(".csv"):
        ...
        if(sframe.columns[0] != "Title"):                       # Fix corrupted 1st Column name we see in some i3-60 files
            CorruptHdr = sframe.columns[0]
            sframe.rename(columns={CorruptHdr: 'Title'}, inplace=True)
        #
        row_date = sframe.iloc[0][DateText]                        # get response date from first row
        if (isinstance(row_date,str)):
            if(row_date[4] == "-"):
                #
                #  Date string is yyyy-mm-dd format
                #
                row_date = datetime.strptime(row_date,"%Y-%m-%d")     # Text date yyyy-mm-dd, convert to datetime object
            else:
                #
                ...
                #  
                if (row_date[1] == '/'):
                    row_date = '0' + row_date                         # single digit month, make 0n
                if (row_date[4] == '/'):
                    row_date = row_date[0:3] + '0' + row_date[3:]        # single digit day, make 0n
                if (len(row_date) == 8):
                    row_date = row_date[0:6] + '20' + row_date[6:]        # two digit year make 20nn
                row_date = datetime.strptime(row_date,"%m/%d/%Y")     # Text date mm/dd/yyyy, convert to datetime object
        else:
            row_date = row_date.to_pydatetime()                       # convert pandas timestamp to datetime object
        svystart = row_date                                        # init start and end dates as first row's date
        svyend = row_date
        #
        ...
        for row in range(0,NumSvyRows):
            ...
            row_cols = list(sframe.iloc[row])                      # get row data as a list
            row_date = row_cols[DateCol]                              # get response date from next row
            #
            ...
            if (NumPersOpts > 0):
                ...
                for q in range(0,NumQuestions):
                    for ii in range(0,NumPersOpts):
                        if (row_cols[FirstQCol+q] == PersOpts[ii]):
            ...
            if (isinstance(row_date,str)):
                ...
                row_date = row_date.to_pydatetime()                   # date is pandas Timestamp, convert to datetime object
            if (row_date < svystart):
                svystart = row_date                                # older than start, so new start date
            if (row_date > svyend):
                svyend = row_date                                  # newer than end date, so new end date
            DowIndex = row_date.weekday()                          # get day of week from datetime object (0 = Monday, 1 = tuesday etc)
            ...
            #
            if(row_cols[PhoneCol] != ""):                          # don't check blank phone numbers
                if row_cols[PhoneCol] in PhoneNumber:
                    PhoneDuplicates += 1                        # this is a duplicate Phone Number
                else:
                    PhoneNumber.append(row_cols[PhoneCol])         # Unique Phone Number so far, add to table
            ...
            #
            rtext = row_cols[ResponseCol]                          # fetch response text
            ...
            #
            try:
                Vname = row_cols[VolFirstCol] + " " + row_cols[VolLastCol] # get volunteer name
            except:
            ...
            if (rtext == Take):
                ...
                if (doquestions(row_cols) != 0):               # process question answers
    ...
    #    the ReportTitle and placed in the directory specified by 'datadir'/reports 
    ...
    if ("SurveyResponse" in rptfile):
        ...
        svyfile2 = datadir + "surveys/" + title + "-" + yyyymmdd + ".csv"    # rename and move survey file
