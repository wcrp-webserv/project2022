...
sub main {
  ...
  NEW:
    ...
    while ( $line1Read = $csv->getline($inputFileh) ) {
        ...
        $UCword                = $csvRowHash{"ResidentialAddress1"};
        $UCword =~ s/(\w+)/\u\L$1/g;
        $baseLine{"Address1"} = $UCword;
        my @streetno = split( / /, $UCword, 2 );
        $baseLine{"StreetNo"}   = $streetno[0];
        $baseLine{"StreetName"} = $streetno[1];
        $UCword                 = $csvRowHash{"ResidentialCity"};
        $UCword =~ s/(\w+)/\u\L$1/g;
        $baseLine{"City"}  = $UCword;
        $baseLine{"State"} = $csvRowHash{"ResidentialState"};
        $baseLine{"Zip"}   = $csvRowHash{"ResidentialZip"};
        $baseLine{"email"} = "";
